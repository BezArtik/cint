...
void type_checker::check_var_declaration(const ast::var_declaration& stmt) {
    ...
    if (stmt.type_.is_void()) {
        reporter_.error(stmt.line_, stmt.column_, err::void_variable);
        return;
    }

    if (symbols_.contains_in_current_scope(name)) {
        reporter_.error(stmt.line_, stmt.column_, err::redeclaration_variable, name);
        return;
    }
    ...
    if (stmt.initializer_) {
		...
		if (stmt.type_.is_array() && init_type.is_array()) {
			if (!stmt.type_.is_assignable_from(init_type)) {
				reporter_.error(stmt.line_, stmt.column_, err::type_mismatch_initialization, name);
				return;
			}
...
void type_checker::check_while(const ast::while_stmt& stmt) {
    ...
    if (cond_type != t::bool_type() && !cond_type.is_unknown()) {
        reporter_.error(stmt.line_, stmt.column_, err::condition_not_bool);
    }
...
void type_checker::check_return_stmt(const ast::return_stmt& stmt) {
    if (!curr_return_type_) {
        reporter_.error(stmt.line_, stmt.column_, err::return_outside_function);
        return;
    }

    if (!stmt.value_) {
        if (!curr_return_type_->is_void()) {
            reporter_.error(stmt.line_, stmt.column_, err::return_missing_value);
        }
        return;
    }
    ...
    if (!curr_return_type_->is_assignable_from(return_type)) {
            reporter_.error(stmt.line_, stmt.column_, err::return_type_mismatch);
    }
}
...
void type_checker::check_func_declaration(const ast::func_declaration& stmt) {
    ...
    if (symbols_.contains_in_current_scope(name)) {
        reporter_.error(stmt.line_, stmt.column_, err::redeclaration_function, name);
        return;
    }
...
t type_checker::type_of_literal(const ast::literal_expr& expr) {
    ...
    reporter_.error(token.line_, token.column_, err::unexpected_literal);
...
t type_checker::type_of_variable(const ast::variable_expr& expr_) {
    ...
    if (!info) {
        reporter_.error(expr_.line_, expr_.column_, err::undefined_variable, name);
        return t::unknown_type();
    }
    return info->type_;
}
...
t type_checker::type_of_binary(const ast::binary_expr& expr) {
    ...
    if (op == tt::EQUAL) {
        if (!left.is_assignable_from(right) || !is_lvalue(expr.left_)) {
            reporter_.error(expr.line_, expr.column_, err::type_mismatch_assignment);
            return t::unknown_type();
        }
        return left;
    }
    ...
    if (op == tt::PLUS_EQUAL || op == tt::MINUS_EQUAL ||
        ...
        if (!is_lvalue(expr.left_)) {
            reporter_.error(expr.line_, expr.column_, err::compound_requires_lvalue);
            return t::unknown_type();
        }

        if (!left.is_numeric() || !right.is_numeric()) {
            reporter_.error(expr.line_, expr.column_, err::compound_requires_numeric);
            return t::unknown_type();
        }
        return left;
    }
    ...
    if (op == tt::PLUS || op == tt::MINUS ||
        ...
        op == tt::PERCENT) {
        if (!left.is_numeric() || !right.is_numeric()) {
            reporter_.error(expr.line_, expr.column_, err::arithmetic_requires_numeric);
            return t::unknown_type();
        }
    ...
    if (op == tt::EQUAL_EQUAL || op == tt::BANG_EQUAL ||
        ...
        if (!left.is_numeric() || !right.is_numeric()) {
            reporter_.error(expr.line_, expr.column_, err::comparison_requires_numeric);
            return t::unknown_type();
        }
        return t::bool_type();
    }
    ...
    if (op == tt::AND || op == tt::OR) {
        if (left != t::bool_type() || right != t::bool_type()) {
            reporter_.error(expr.line_, expr.column_, err::logical_requires_bool);
            return t::unknown_type();
        }
        return t::bool_type();
    }

    reporter_.error(expr.line_, expr.column_, err::unsupported_binary_operator);
    return t::unknown_type();
}
...
t type_checker::type_of_unary(const ast::unary_expr& expr) {
    ...
    if (op == tt::MINUS) {
        if (!operand_type.is_numeric()) {
            reporter_.error(expr.line_, expr.column_, err::unary_minus_requires_numeric);
            return t::unknown_type();
        }
        return operand_type;
    }
    ...
    if (op == tt::INCREMENT || op == tt::DECREMENT) {
        if (!operand_type.is_numeric()) {
            reporter_.error(expr.line_, expr.column_, err::increment_requires_numeric);
            return t::unknown_type();
        }
        if (!is_lvalue(expr.operand_)) {
            reporter_.error(expr.line_, expr.column_, err::increment_requires_lvalue);
            return t::unknown_type();
        }
        return operand_type;
    }
    ...
    if (op == tt::BANG) {
        if (operand_type != t::bool_type()) {
            reporter_.error(expr.line_, expr.column_, err::not_requires_bool);
            return t::unknown_type();
        }
        return t::bool_type();
    }

    reporter_.error(expr.line_, expr.column_, err::unsupported_unary_operator);
    return t::unknown_type();
}
...
t type_checker::type_of_call(const ast::call_expr& expr) {
    ...
    for (const auto& def : core::builtins) {
        ...
        reporter_.error(expr.line_, expr.column_, err::no_matching_overload, name);
        return t::unknown_type();
    }
    ...
    if (!info || info->kind_ != symbol_kind::FUNCTION) {
        reporter_.error(expr.line_, expr.column_, err::undefined_function, name);
        return t::unknown_type();
    }
    ...
    if (expr.args_.size() != param_types.size()) {
        reporter_.error(expr.line_, expr.column_,
            err::argument_count_mismatch, name,
    ...
    for (size_t i = 0; i < expr.args_.size(); i++) {
        ...
        if (!param_types[i].is_assignable_from(arg_type)) {
            reporter_.error(expr.line_, expr.column_, 
                err::argument_type_mismatch, i + 1, name);
            return t::unknown_type();
        }
    }
...
t type_checker::type_of_array_literal(const ast::array_literal_expr& expr) {
    ...
    for (size_t i = 1; i < expr.elements_.size(); i++) {
        ...
        if (t.is_unknown()) return t::unknown_type();
        if (!elem_type.is_assignable_from(t)) {
            reporter_.error(expr.line_, expr.column_, err::array_literal_inconsistent_types);
            return t::unknown_type();
        }
    }
...
t type_checker::type_of_index(const ast::index_expr& expr) {
    ...
    if (!object_type.is_array()) {
        reporter_.error(expr.line_, expr.column_, err::indexing_non_array);
        return t::unknown_type();
    }
	if (!index_type.is_numeric()) {
		reporter_.error(expr.line_, expr.column_, err::indexing_non_numeric);
		return t::unknown_type();
	}
