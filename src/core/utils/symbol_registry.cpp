...
symbol_registry symbol_registry::build(std::span<const ast::stmt_ptr> ast, std::span<const builtin_def> builtins) {
    ...
    for (const auto& stmt : ast) registry.add_ast_entry(stmt);
...
type symbol_registry::resolve_type(const type& t) const {
    if (t.is_struct()) {
        if (!t.struct_fields().empty()) {
            ...
            for (const auto& [name, field_type] : t.struct_fields())
        ...
        if (entry && std::holds_alternative<struct_ptr>(entry->info_)) return resolve_type(entry->type_);
