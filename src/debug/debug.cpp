...
void print_statement(const debug_writer& writer, const ast::statement& stmt, uint32_t level,
    ...
    core::visit(
        core::overloaded{
            [&](const ast::node<ast::expression_stmt>& s) {
            ...
            [&](const ast::node<ast::return_stmt>& s) {
                if (s->value_) {
                    header << "ReturnStmt\n";
                    writer.emit(header.str());
