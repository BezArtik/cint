 ...
 *
 * Реализовано как **type-erased обёртка** над std::variant. Выбор variant
 * (а не иерархии классов с виртуальными функциями) обусловлен:
 * - Отсутствием динамических выделений для примитивных типов
 * - Эффективным доступом через std::get_if / std::visit
 * - Компактным хранением: variant занимает размер наибольшего типа + дискриминант
 *
 * Ссылочные типы (string, array) хранятся через shared_ptr для:
 * - Дешёвого копирования (строки и массивы не клонируются при присваивании)
 * - Семантики ссылок (изменение через один value видно через другой)
 *
 * Преобразования типов:
 * - Неявное: int → double (is_assignable_from разрешает)
 * - Явное: через convert() с указанием целевого типа
 * - Из строки: from_string() для парсинга литералов
 *
